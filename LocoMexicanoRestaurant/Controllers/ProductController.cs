        ...
        public async Task<IActionResult> AddEdit(Product product, int[] ingredientIds, int catId)
        {
			...
			if (ModelState.IsValid)
            {
                if (product.ImageFile != null)
                {
                    ...
                    string uniqueFileName = Guid.NewGuid().ToString() + product.ImageFile.FileName;
				...
				{
                    //ef didnt recognize product passed from the view so fix is to re-hit the db
                    var existingProduct = await products.GetByIdAsync(
                        product.ProductId, new QueryOptions<Product> { Includes = "ProductIngredients" });
                    ...
                    foreach (var id in ingredientIds)
                    {
                        existingProduct.ProductIngredients?.Add(
                            new ProductIngredient { IngredientId = id, ProductId = product.ProductId });
                    }
                    try
                    {
